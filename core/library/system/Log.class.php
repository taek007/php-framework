...
class Log { 
    ...
    static public function init($config=array()){
        ...
        $class  =   strpos($type,'\\')? $type: 'system\\driver\\driver\\log\\'. ucwords(strtolower($type));           
        unset($config['type']);
    ...
    static function save($type='',$destination='') {
        ...
        if(!self::$storage){
            ...
            $class  =   'system\\driver\\log\\'. ucwords($type);
    ...
    static function write($message,$level=self::ERR,$type='',$destination='') {
        ...
        if(empty($destination))
            $destination = C('LOG_PATH').date('y_m_d').'.log';   
        if(is_array($message)){
            self::$storage->write("{$level}: ".var_export($message,true), $destination);
        }else{
