...
class Pager { 
    ...
    protected $preCss = "i_page_pre";
    protected $nextCss= "i_page_next";
    ...
    public function show() { 
        ...
        if ($upRow > 0) {
            if ($this->type == "ajax") {
                ...
                if ($upRow == 1) {
                    $upPage = '<a class="'.$this->preCss.'"  href="' . $this->rule['firstpage'] . '">' . $this->config ['prev'] . '</a>';
                } else {
                    $upPage = '<a  class="'.$this->preCss.'"  href="' . $this->rule['first'] . $upRow . $this->rule['end'] . '">' . $this->config ['prev'] . '</a>';
                }
            } else if ($this->type == "url") {
                $upPage = '<a  class="'.$this->preCss.'"  href="' . $url . $split . $p . '=' . $upRow . '">' . $this->config ['prev'] . '</a>';
            }
        } else {
        ...
        if ($downRow <= $this->totalPages) {
            if ($this->type == "ajax") {
                ...
                $downPage = '<a  class="'.$this->nextCss.'"  href="' . $this->rule['first'] . $downRow . $this->rule['end'] . '">' . $this->config ['next'] . '</a>';
            } else if ($this->type == "url") {
                $downPage = '<a  class="'.$this->nextCss.'" href="' . $url . $split . $p . '=' . $downRow . '">' . $this->config ['next'] . '</a>';
            }
        } else {
            if($this->totalPages!=1){
...
//                } 
                if ($page != $this->nowPage) {
                    ...
                    if ($this->totalPages != 1) {
                        $linkPage .= '<a href="javascript:" class="' . $this->curr . '">' . $page . '</a>';
                    }
                }
            }
            if ($this->split) { 
